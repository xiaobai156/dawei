...
def baoma_xuanji_candidates(
    ...
    for index in range(start_index + 1, document_range.stop):
        ...
        if (
            index > start_index + 1
            and "必中12码肖" not in line
            and not BAOMA_XUANJI_RE.search(line)
            and any(keyword in line for keyword in NUMBER_BLOCK_STOP_KEYWORDS)
        ):
            break
...
def extract_baoma_xuanji(text_or_html: str, config: SiteConfig) -> SiteResult:
    return extract_from_candidates(
        ...
        no_candidates_message="宝马玄机综合特码栏目没有找到符合条件的数据",
        issue_range_error=(
...
def extract_zhuchiren_tab(text_or_html: str, config: SiteConfig) -> SiteResult:
    return extract_from_candidates(
        ...
        no_candidates_message=(
            f"{config.name}{zhuchiren_tab_anchor(config)}栏目没有找到符合条件的数据"
        ),
        issue_range_error=(
            f"no 36-number record found in issue range {DEFAULT_ISSUE_MIN}-{DEFAULT_ISSUE_MAX}"
        ),
...
def extract_xiaoyuer(text_or_html: str, config: SiteConfig) -> SiteResult:
    return extract_from_candidates(
        ...
        config,
        xiaoyuer_candidates,
        no_candidates_message="小鱼儿澳门精准36码表格没有找到符合条件的数据",
        issue_range_error=(
            f"no 36-number record found in issue range {DEFAULT_ISSUE_MIN}-{DEFAULT_ISSUE_MAX}"
        ),
    )
